...
export class TowerOfHanoi<T> {
    ...
    public solve(
        ...
        // Извлекаем объект из исходной башни
        let movedObj = from.stack.pop();

        // Логируем перемещение объекта с использованием JSON.stringify()
        console.log(`Сущность ${JSON.stringify(movedObj)} перемещена из башни ${from.name} в башню ${to.name}`);

        // Перемещаем объект в целевую башню
    ...
    public checkLastTower(): void {
        ...
        console.log(`Последняя башня: ${JSON.stringify(tower)}`);
    }
}
